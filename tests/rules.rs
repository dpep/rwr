...
/// Every shipped rule does what it says, asserted by its own fixtures.
///
/// These expectations used to live here as two Rust tables -- maintained in a
/// file a rule author editing `rules/performance/detect.yml` would never open.
/// They are `tests:` blocks in the rules now, so a rule and its proof travel
/// together, and this is the one line that runs them all. Two sources of truth
/// for one behaviour is the drift the fixtures exist to prevent, and it applied
/// to itself.
#[test]
fn the_shipped_pack_passes_its_own_fixtures() {
    let out = rwr(&["test", "all"]);
    assert_eq!(
        out.status.code(),
        Some(0),
        "{}{}",
    ...
    // Every rule carries fixtures. A partly-covered pack must not read like a
    // covered one, so this fails when a new rule arrives without them.
    assert!(
        !String::from_utf8_lossy(&out.stdout).contains("no fixtures"),
        "{}",
        String::from_utf8_lossy(&out.stdout)
    );
}
